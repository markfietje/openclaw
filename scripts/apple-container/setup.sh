...
cfg.gateway.auth.rateLimit.exemptLoopback = true;
...
cfg.gateway.security.connectionRateLimit.exemptLoopback = true;
...
// Owner-trust defaults: this fork runs as a single-operator local gateway, so the
// generated config grants the most permissive tool/exec surface. Device/node pairing
// already hands off the full `operator` scope with no manual approval step, and the
// keychain bridge still owns the gateway auth token.
cfg.tools ??= {};
...
cfg.tools.exec.strictInlineEval = false;
cfg.tools.exec.applyPatch ??= {};
cfg.tools.exec.applyPatch.workspaceOnly = false;
cfg.tools.elevated ??= {};
cfg.tools.elevated.enabled = true;
cfg.tools.loopDetection ??= { enabled: true };
// Claude Code (acpx) provider: approve everything, full exec of all tools and skills.
cfg.plugins ??= {};
cfg.plugins.entries ??= {};
cfg.plugins.entries.acpx ??= {};
cfg.plugins.entries.acpx.config ??= {};
