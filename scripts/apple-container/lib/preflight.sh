...
preflight_check_security() {
  if ! command -v /usr/bin/security >/dev/null 2>&1; then
    preflight_fail "macOS Keychain CLI (/usr/bin/security)" "Expected at /usr/bin/security"
    return 1
  fi
  preflight_pass "macOS Keychain CLI" "/usr/bin/security"
}
...
preflight_check_launchctl() {
  if command -v launchctl >/dev/null 2>&1; then
    preflight_pass "launchctl" "bridge auto-restart"
  else
    preflight_skip "launchctl" "Bridge will use nohup fallback"
  fi
}
...
preflight_check_keychain_token() {
  local service="${1:-ai.openclaw.apple-container.gateway-token}"
  local account="${2:-${USER:-openclaw}}"
  if ! /usr/bin/security find-generic-password -a "$account" -s "$service" -w >/dev/null 2>&1; then
    preflight_fail "Keychain token" "Run: scripts/apple-container/setup.sh (service '${service}' / account '${account}')"
    return 1
  fi
  preflight_pass "Keychain token" "${service} / ${account}"
}
...
preflight_run_setup() {
  ...
  preflight_check_node
  preflight_check_token_source
  preflight_check_keychain_token "${2:-}" "${3:-}"
