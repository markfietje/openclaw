...
dashboard_url() {
  ...
  if [[ -n "$base_url" ]]; then
    printf '%s' "$token" | OPENCLAW_DASHBOARD_BASE_URL="$base_url" node -e '
let token = "";
...
'
    ...
    printf '%s' "$token" |
      OPENCLAW_DASHBOARD_PORT="$HOST_PORT" node -e '
let token = "";
process.stdin.setEncoding("utf8");
...
process.stdin.on("end", () => {
  const url = new URL(`http://127.0.0.1:${process.env.OPENCLAW_DASHBOARD_PORT || "18789"}/`);
  url.hash = new URLSearchParams({ token: token.trim() }).toString();
  process.stdout.write(`${url.toString()}\n`);
});
'
...
if (!cfg.gateway.trustedProxies.includes(desiredCidr)) {
  ...
  cfg.gateway.trustedProxies = cfg.gateway.trustedProxies.filter(
    (e) => !/^192\.168\.(6[4-9]|7[0-9])\.0\/24$/.test(e)
  );
...
SYNCNODE
  ...
  if [[ "$changed" == "1" ]]; then
    ...
    # Re-stage the updated host config into the container volume so the
    # gateway picks up the correct trustedProxies on next start.
...
do_run() {
  ...
  if [[ "${#detach_arg[@]}" -gt 0 ]]; then
    ...
    if [[ "$http_code" == "200" ]]; then
      ...
      if is_enabled "$open_after_start"; then
        open_dashboard
      fi
    else
