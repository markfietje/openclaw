...
OVERLAY_SUBPATHS=(dist skills qa docs openclaw.mjs package.json)
...
if $FULL_SEED; then
  ...
  if ! container run \
    ...
    sh -c '
      ...
      for subpath in dist skills qa docs openclaw.mjs package.json '"$STAMP_FILE"'; do
...
if [[ -n "$SYNC_RUNTIME" ]]; then
  ...
  env OPENCLAW_APPLE_CONTAINER_CODE_VOLUME="$CODE_VOLUME_NAME" "$RUN_SH"
fi
