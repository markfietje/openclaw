...
The origin validation system implements eight distinct security layers, each addressing a specific attack vector. These layers are evaluated in sequence, and the request fails closed at the first layer that rejects it.
...
### Layer 1.5: Fetch Metadata Cross-Site Rejection

As a defense-in-depth measure aligned with the OWASP Fetch Metadata specification, the system inspects the `Sec-Fetch-Site` request header. When a browser sends `Sec-Fetch-Site: cross-site`, the request is rejected outright, regardless of whether the origin is on the allowlist. This provides an additional signal that the request originated from a different site, which is useful for detecting cross-site WebSocket hijacking attempts even in edge cases where origin validation alone might be insufficient. Non-browser clients (CLIs, native apps) typically omit this header entirely and are allowed through:
...
In addition to the origin check layers, the gateway enforces strict validation on security-sensitive HTTP headers via the `validateSensitiveHeaders` function in `src/gateway/net.ts`. This prevents header chain injection attacks where an attacker attempts to smuggle multiple values into a single header field.

The function validates the following headers:

| Header                 | Attack Prevented                       |
| ---------------------- | -------------------------------------- |
| `Host`                 | Virtual host spoofing                  |
| `Origin`               | CSRF / CSWSH origin forgery            |
| `X-Forwarded-Host`     | Proxy host spoofing                    |
| `X-Forwarded-Proto`    | Protocol downgrade                     |
| `X-Forwarded-For`      | Source IP spoofing via chain injection |
| `X-Real-IP`            | Source IP spoofing                     |
| `Forwarded` (RFC 7239) | Combined proxy header spoofing         |

The validation rejects:

- **Duplicate headers**: When a header appears more than once (e.g., two `X-Forwarded-For` values), the request is rejected. This prevents attackers from appending their own IP to an existing chain.
- **Chained values**: Comma-separated values within a single header (e.g., `X-Forwarded-For: 1.2.3.4, 5.6.7.8`) are rejected for security-sensitive headers. This is intentional — while `X-Forwarded-For` chains are legitimate in some proxy configurations, the gateway parses them through its own trust-preserving logic in `parseForwardedChain`, and raw comma-separated values in the pre-validation layer are treated as suspicious.

This strict validation runs in the WebSocket `verifyClient` callback (before the HTTP 101 response) and can be toggled via `gateway.security.strictHeaderValidation` (default: `true`).
