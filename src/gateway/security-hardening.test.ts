...
describe("Security Hardening - Gap 2: Tailscale Loopback Origin Bypass", () => {
  ...
  describe("when disableLocalhostPrivilege is false (direct local, no proxy)", () => {
    ...
    it("accepts same-origin 127.0.0.1 origin as private same-origin", () => {
      const result = checkBrowserOrigin({
        ...baseParams,
        origin: "http://127.0.0.1:18789",
      });
...
describe("Security Hardening - Gap 5: config.set Can Modify Auth Config", () => {
  ...
  describe("isProtectedConfigPath", () => {
    ...
    it("marks gateway.controlUi.allowedOrigins as protected", () => {
      expect(isProtectedConfigPathFn("gateway.controlUi.allowedOrigins")).toBe(true);
    });
  });
  ...
  describe("configObjectModifiesProtectedPath", () => {
    ...
    it("flags gateway.controlUi.allowedOrigins as protected", () => {
      expect(
        configObjectModifiesProtectedPathFn({
    ...
    it("does NOT flag non-protected gateway.controlUi changes", () => {
      expect(
        configObjectModifiesProtectedPathFn({
  ...
  describe("admin:config capability authorization", () => {
    ...
    it("config.set (non-protected) still requires admin:write", () => {
      const ctx = createCtx(["admin:read"]);
      const result = authorizeMessageFn(ctx, "gateway.method.config.set", {
        logDenied: false,
      });
      expect(result.ok).toBe(false);
    });
...
describe("Security Hardening - Scope Translation: operator.* to capabilities", () => {
  let authorizeMessageFn: typeof import("./message-auth.js").authorizeMessage;
  let createMessageAuthContextFn: typeof import("./message-auth.js").createMessageAuthContext;
  ...
  it("operator.admin grants admin:read and admin:write but NOT secrets:read or admin:config", () => {
    const ctx = createCtx(["operator.admin"]);

    // admin:read
    expect(authorizeMessageFn(ctx, "gateway.method.health", { logDenied: false }).ok).toBe(true);

    // admin:write (config.set matches config. prefix fallback)
    expect(authorizeMessageFn(ctx, "gateway.method.config.set", { logDenied: false }).ok).toBe(
      true,
    );

    // secrets:read NOT granted
    const secretsResult = authorizeMessageFn(ctx, "gateway.method.secrets.resolve", {
      logDenied: false,
    });
    expect(secretsResult.ok).toBe(false);
    if (!secretsResult.ok) {
      expect(secretsResult.missingCapability).toBe("secrets:read");
    }

    // admin:config NOT granted
    const protectedResult = authorizeMessageFn(ctx, "gateway.method.config.set_protected", {
      logDenied: false,
    });
    expect(protectedResult.ok).toBe(false);
    if (!protectedResult.ok) {
      expect(protectedResult.missingCapability).toBe("admin:config");
    }
  });

  it("operator.read grants admin:read only", () => {
    const ctx = createCtx(["operator.read"]);

    expect(authorizeMessageFn(ctx, "gateway.method.health", { logDenied: false }).ok).toBe(true);

    const sendResult = authorizeMessageFn(ctx, "gateway.method.send", {
      logDenied: false,
    });
    expect(sendResult.ok).toBe(false);
    if (!sendResult.ok) {
      expect(sendResult.missingCapability).toBe("admin:write");
    }
  ...
  it("operator.write grants admin:read and admin:write (write implies read)", () => {
    const ctx = createCtx(["operator.write"]);

    // admin:read
    expect(authorizeMessageFn(ctx, "gateway.method.health", { logDenied: false }).ok).toBe(true);

    // admin:write
    expect(authorizeMessageFn(ctx, "gateway.method.send", { logDenied: false }).ok).toBe(true);

    // admin:write (sessions.delete is admin:write)
    expect(
      authorizeMessageFn(ctx, "gateway.method.sessions.delete", {
        logDenied: false,
      }).ok,
    ).toBe(true);
  ...
  it("operator.approvals grants admin:write", () => {
    const ctx = createCtx(["operator.approvals"]);

    expect(
      authorizeMessageFn(ctx, "gateway.method.exec.approval.resolve", {
        logDenied: false,
      }).ok,
    ).toBe(true);

    // health requires admin:read, not admin:write
    const healthResult = authorizeMessageFn(ctx, "gateway.method.health", {
      logDenied: false,
    });
    expect(healthResult.ok).toBe(false);
    if (!healthResult.ok) {
      expect(healthResult.missingCapability).toBe("admin:read");
    }
  });

  it("operator.pairing grants admin:write", () => {
    const ctx = createCtx(["operator.pairing"]);

    expect(
      authorizeMessageFn(ctx, "gateway.method.device.pair.list", {
        logDenied: false,
      }).ok,
    ).toBe(true);
  ...
  it("direct capability scopes work alongside operator.* scopes", () => {
    // operator.admin + explicit secrets:read
    const ctxSecrets = createCtx(["operator.admin", "secrets:read"]);
    expect(
      authorizeMessageFn(ctxSecrets, "gateway.method.secrets.resolve", {
        logDenied: false,
      }).ok,
    ).toBe(true);

    // operator.admin + explicit admin:config
    const ctxConfig = createCtx(["operator.admin", "admin:config"]);
    expect(
      authorizeMessageFn(ctxConfig, "gateway.method.config.set_protected", {
        logDenied: false,
      }).ok,
    ).toBe(true);
  });

  it("wildcard * scope bypasses everything", () => {
    const ctx = createCtx(["*"]);

    expect(authorizeMessageFn(ctx, "gateway.method.health", { logDenied: false }).ok).toBe(true);
    expect(
      authorizeMessageFn(ctx, "gateway.method.secrets.resolve", {
        logDenied: false,
      }).ok,
    ).toBe(true);
    expect(
      authorizeMessageFn(ctx, "gateway.method.config.set_protected", {
        logDenied: false,
      }).ok,
    ).toBe(true);
    expect(authorizeMessageFn(ctx, "gateway.method.send", { logDenied: false }).ok).toBe(true);
  ...
  it("macOS app default scopes work correctly (no secrets, no admin:config)", () => {
    const ctx = createCtx([
      "operator.admin",
      "operator.read",
      "operator.write",
    ...
    // admin:write (config.set via prefix fallback)
    expect(authorizeMessageFn(ctx, "gateway.method.config.set", { logDenied: false }).ok).toBe(
      true,
    );

    // secrets:read NOT granted — even with all operator.* scopes
    const secretsResult = authorizeMessageFn(ctx, "gateway.method.secrets.resolve", {
      logDenied: false,
    });
    expect(secretsResult.ok).toBe(false);
    ...
    // admin:config NOT granted — even with all operator.* scopes
    const protectedResult = authorizeMessageFn(ctx, "gateway.method.config.set_protected", {
      logDenied: false,
    });
    expect(protectedResult.ok).toBe(false);
...
describe("Security Hardening - Real Method Name Capability Mapping", () => {
  let authorizeMessageFn: typeof import("./message-auth.js").authorizeMessage;
  let createMessageAuthContextFn: typeof import("./message-auth.js").createMessageAuthContext;
  ...
  describe("read methods require admin:read", () => {
    const readMethods = [
    ...
    it.each(readMethods)("%s maps to admin:read", (method) => {
      const ctx = createCtx(["admin:read"]);
      const result = authorizeMessageFn(ctx, method, { logDenied: false });
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.capability).toBe("admin:read");
      }
    });

    it.each(readMethods)("%s is denied without admin:read", (method) => {
      const ctx = createCtx(["admin:write"]);
      const result = authorizeMessageFn(ctx, method, { logDenied: false });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.missingCapability).toBe("admin:read");
      }
    });
  });

  describe("write methods require admin:write", () => {
    const writeMethods = [
      "gateway.method.send",
      "gateway.method.chat.send",
      "gateway.method.agent",
      "gateway.method.sessions.create",
    ];

    it.each(writeMethods)("%s maps to admin:write", (method) => {
      const ctx = createCtx(["admin:write"]);
      const result = authorizeMessageFn(ctx, method, { logDenied: false });
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.capability).toBe("admin:write");
      }
    });

    it.each(writeMethods)("%s is denied with only admin:read", (method) => {
      const ctx = createCtx(["admin:read"]);
      const result = authorizeMessageFn(ctx, method, { logDenied: false });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.missingCapability).toBe("admin:write");
      }
    });
  });

  describe("admin methods require admin:write", () => {
    const adminMethods = [
      "gateway.method.channels.logout",
      "gateway.method.agents.create",
      "gateway.method.cron.add",
      "gateway.method.sessions.delete",
    ];

    it.each(adminMethods)("%s maps to admin:write", (method) => {
      const ctx = createCtx(["admin:write"]);
      const result = authorizeMessageFn(ctx, method, { logDenied: false });
      expect(result.ok).toBe(true);
      if (result.ok) {
  ...
  it("node.invoke requires admin:write", () => {
    const ctx = createCtx(["admin:write"]);
    const result = authorizeMessageFn(ctx, "gateway.method.node.invoke", {
      logDenied: false,
    });
    expect(result.ok).toBe(true);
    if (result.ok) {
    ...
    const deniedCtx = createCtx(["admin:read"]);
    const deniedResult = authorizeMessageFn(deniedCtx, "gateway.method.node.invoke", {
      logDenied: false,
    });
    expect(deniedResult.ok).toBe(false);
    if (!deniedResult.ok) {
      expect(deniedResult.missingCapability).toBe("admin:write");
    }
  });

  describe("pairing methods require admin:write", () => {
    const pairingMethods = ["gateway.method.device.pair.list", "gateway.method.node.pair.approve"];

    it.each(pairingMethods)("%s maps to admin:write", (method) => {
      const ctx = createCtx(["admin:write"]);
      const result = authorizeMessageFn(ctx, method, { logDenied: false });
      expect(result.ok).toBe(true);
      if (result.ok) {
  ...
  describe("secrets methods require separate capabilities", () => {
    it("secrets.resolve requires secrets:read", () => {
      const ctx = createCtx(["secrets:read"]);
      const result = authorizeMessageFn(ctx, "gateway.method.secrets.resolve", {
        logDenied: false,
      });
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.capability).toBe("secrets:read");
      }

      // admin:write does NOT grant secrets:read
      const adminCtx = createCtx(["admin:write"]);
      const adminResult = authorizeMessageFn(adminCtx, "gateway.method.secrets.resolve", {
        logDenied: false,
      });
      expect(adminResult.ok).toBe(false);
      if (!adminResult.ok) {
        expect(adminResult.missingCapability).toBe("secrets:read");
      }
    });

    it("secrets.reload requires secrets:manage", () => {
      const ctx = createCtx(["secrets:manage"]);
      const result = authorizeMessageFn(ctx, "gateway.method.secrets.reload", {
        logDenied: false,
      });
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.capability).toBe("secrets:manage");
      }

      // secrets:read does NOT grant secrets:manage
      const readCtx = createCtx(["secrets:read"]);
      const readResult = authorizeMessageFn(readCtx, "gateway.method.secrets.reload", {
        logDenied: false,
      });
      expect(readResult.ok).toBe(false);
      if (!readResult.ok) {
        expect(readResult.missingCapability).toBe("secrets:manage");
      }
    });
  });
});

describe("Wildcard origin detection", () => {
  it("detects wildcard in allowedOrigins config", () => {
    const config = { gateway: { controlUi: { allowedOrigins: ["*"] } } };
