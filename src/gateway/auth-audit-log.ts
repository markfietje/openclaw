...
const BASE_FILENAME = "gateway-auth";
const EXT = "jsonl";
...
/**
 * Verify an audit log line against a signing token.
 *
...
export function verifyLine(
  ...
  if (!token) {
    ...
    return { valid: true, entry: parsed as unknown as AuthAuditEntry };
  }
  ...
  return { valid: true, entry: rest as unknown as AuthAuditEntry };
}
...
export function createAuthAuditLogger(config?: AuthAuditLogConfig): AuthAuditLogger {
  ...
  async function writeEntry(entry: Omit<AuthAuditEntry, "ts">): Promise<void> {
    const line: AuthAuditEntry = { ts: new Date().toISOString(), ...entry };
    ...
    await ensureDir();
    await appendFile(activeFile, data, "utf8");
