...
describe("resolveClientIp", () => {
  it.each([
    ...
    {
      name: "returns loopback remote when all X-Forwarded-For hops are trusted proxies and remote is loopback",
      remoteAddr: "127.0.0.1",
    ...
    {
      name: "returns loopback remote when trusted proxy omits forwarding headers (direct loopback connection)",
      remoteAddr: "127.0.0.1",
      trustedProxies: ["127.0.0.1"],
      expected: "127.0.0.1",
    },
    {
      name: "returns loopback remote when X-Forwarded-For entries are all invalid",
      remoteAddr: "127.0.0.1",
    ...
    {
      name: "returns loopback remote when X-Real-IP is not trusted by default",
      remoteAddr: "127.0.0.1",
      realIp: "[2001:db8::5]",
      trustedProxies: ["127.0.0.1"],
      expected: "127.0.0.1",
