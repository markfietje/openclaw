...
describe("checkBrowserOrigin", () => {
  it.each([
    ...
    {
      name: "rejects wildcard allowlists",
      input: {
  ...
  describe("Sec-Fetch-Site rejection (OWASP Fetch Metadata)", () => {
    const baseParams = {
      requestHost: "127.0.0.1:18789",
      origin: "https://gateway.tailnet.ts.net",
      allowedOrigins: ["https://gateway.tailnet.ts.net"],
    ...
    it("rejects cross-site requests when Sec-Fetch-Site is cross-site", () => {
      const result = checkBrowserOrigin({
        ...baseParams,
      ...
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.reason).toBe("cross-site request rejected");
      }
    });

    it("rejects cross-site with mixed case", () => {
      const result = checkBrowserOrigin({
        ...baseParams,
        secFetchSite: "Cross-Site",
      });
    ...
    it("allows requests without Sec-Fetch-Site (non-browser clients)", () => {
      const result = checkBrowserOrigin({
        ...baseParams,
    ...
    it("allows requests with empty Sec-Fetch-Site", () => {
      const result = checkBrowserOrigin({
        ...baseParams,
        secFetchSite: "",
      });
    ...
    it("allows requests with whitespace-only Sec-Fetch-Site", () => {
      const result = checkBrowserOrigin({
        ...baseParams,
        secFetchSite: "   ",
      });
    ...
    it("rejects cross-site even when origin is in allowlist", () => {
      // The whole point: even a valid allowlisted origin is rejected if the browser
      // explicitly says this is a cross-site request (Fetch Metadata defense-in-depth).
      const result = checkBrowserOrigin({
        ...baseParams,
        secFetchSite: "cross-site",
      });
    ...
    it("takes priority over host-header fallback", () => {
      const result = checkBrowserOrigin({
        ...baseParams,
        secFetchSite: "cross-site",
        allowHostHeaderOriginFallback: true,
    ...
    it("takes priority over local loopback exemption", () => {
      const result = checkBrowserOrigin({
        ...baseParams,
        secFetchSite: "cross-site",
        isLocalClient: true,
